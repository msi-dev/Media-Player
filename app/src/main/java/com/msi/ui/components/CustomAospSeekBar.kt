...
fun CustomAospSeekBar(
    ...
    // Theme references for Dynamic Colors (Material You)
    val primaryColor = MaterialTheme.colorScheme.primary
    val trackBgColor = MaterialTheme.colorScheme.onSurfaceVariant.copy(alpha = 0.24f)
    val primaryCopy35 = primaryColor.copy(alpha = 0.35f)

    // Icons to draw inside the thumb
    val musicNotePainter = rememberVectorPainter(Icons.Filled.MusicNote)
    ...
    BoxWithConstraints(
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            val trackHeight = 10.dp.toPx() // Chonky Material You style track
            val thumbRadius = if (isDragging) 14.dp.toPx() else 11.dp.toPx()
            val canvasWidth = size.width
            val canvasHeight = size.height

            // 1. Draw Background Track (Rounded Material You style background track)
            drawRoundRect(
                color = trackBgColor,
            ...
            // 2. Draw Active Track (Rounded active track overlay)
            ...
            // 3. Draw thumb glow during active dragging
            ...
            // 4. Draw Core Primary-colored Circle Thumb
            drawCircle(
                color = primaryColor,
                radius = thumbRadius,
                center = Offset(thumbX, centerY)
            ...
            // 5. Draw the Music Note Icon inside the Seekbar Thumb!
            val iconSize = (thumbRadius * 1.3f).coerceAtMost(22.dp.toPx())
            translate(left = thumbX - iconSize / 2, top = centerY - iconSize / 2) {
                with(musicNotePainter) {
                    draw(
